...
public class ChangeArrayLength 
{
   ...
   public static int[] changeLength1D(int[] element,
                                 int n, int newLength)
   {
      ...
      Object [] newArray = (Object []) Array.newInstance
                (element.getClass().getComponentType(), newLength);
   
      // copy from old space to new space
      System.arraycopy(element, 0, newArray, 0, n);
   
      return newArray;
   }

   /* Change the length of a 1D array with a.length elements */
   public static int[] changeLength1D(int[] element, int newLength)
      {return changeLength1D(element, element.length, newLength);}
